...
const Comissao: React.FC = () => {
    ...
    const [representantes, setRepresentantes] = useState<Representante[]>([{ id: 1, percentual: "", tipo: "Representante 1" }]);
    ...
    const atualizarTabela = useCallback(async () => {
        ...
        try {
            const response = await apiPost("/api/comissao/pesquisar", { duplicatas: duplicatasConsulta });
    ...
    const getAvailableOptions = (currentId: number) => {
        const selecionados = representantes.filter((rep) => rep.id !== currentId).map((rep) => rep.tipo).filter(Boolean);
    ...
    const handlePesquisar = async () => {
        ...
        try {
            ...
            if (response.success && response.data?.rows) {
                ...
                if (naoEncontradas > 0) toastService.warn(`${naoEncontradas} não encontrada(s).`);
    ...
    const adicionarRepresentante = () => {
        if (representantes.length < 3) setRepresentantes([...representantes, { id: Date.now(), percentual: "", tipo: "" }]);
    };

    const atualizarRepresentante = (id: number, campo: "tipo" | "percentual", valor: string) => {
        setRepresentantes(representantes.map(rep => (rep.id === id ? { ...rep, [campo]: valor } : rep)));
    };
        ...
        <div className="comissao-container">
            <div className="comissao-controles-superiores">
                <div className="comissao-painel-consulta">
                    ...
                    <textarea id="consulta-duplicatas" value={consultaInput} onChange={(e) => setConsultaInput(e.target.value)} placeholder="Informe os números separados por vírgula ou quebra de linha" className="comissao-textarea" disabled={loading} />
                    <button className="comissao-btn primary" onClick={handlePesquisar} disabled={loading || !consultaInput.trim()}><i className="fas fa-search"></i> Consultar</button>
                </div>
                        ...
                        <div className="comissao-representantes-container">
                            {representantes.map((rep) => (
                                <div key={rep.id} className="comissao-representante-row">
                                    <select className="comissao-select" value={rep.tipo} onChange={(e) => atualizarRepresentante(rep.id, "tipo", e.target.value)} disabled={loading}>
                                        <option value="">Selecione...</option>
                                        {getAvailableOptions(rep.id).map((opt) => (<option key={opt} value={opt}>{opt}</option>))}
                                    </select>
                                    <input type="number" value={rep.percentual} onChange={(e) => atualizarRepresentante(rep.id, "percentual", e.target.value)} placeholder="%" className="comissao-input-percentual" disabled={loading || !rep.tipo} min={0} step={0.01} />
                                </div>
                            ))}
                            {representantes.length < 3 && <button className="comissao-btn adicionar" onClick={adicionarRepresentante} disabled={loading}><i className="fas fa-plus"></i> Adicionar</button>}
                        </div>
                        <hr />
                        <button className="comissao-btn primary full-width" onClick={handleAjustarComissao} disabled={loading || duplicatasSelecionadas.size === 0 || representantes.filter(r => r.tipo && r.percentual).length === 0}>
                            <i className="fas fa-exchange-alt"></i> Ajustar ({duplicatasSelecionadas.size}) Selecionada(s)
                        </button>
                        <button className="comissao-btn secondary full-width" onClick={handleLimpar} disabled={loading}><i className="fas fa-eraser"></i> Limpar Tudo</button>
                    </div>
                </div>
            </div>
            ...
            {duplicatas.length > 0 && (
                <div className="comissao-table-card">
                    <div className="comissao-table-header">
                        ...
                        {duplicatasAjustadas.size > 0 && <button className="comissao-btn tertiary" onClick={() => setIsReleaseModalOpen(true)}><i className="fas fa-unlock"></i> Liberar Duplicatas</button>}
                    </div>
                        ...
                        <table className="comissao-table">
                            <thead>
                                <tr>
                                    ...
                                    <th>Nº Duplicata</th><th>Cliente</th><th>Rep. 1</th><th>% Com. 1</th><th>Rep. 2</th><th>% Com. 2</th><th>Rep. 3</th><th>% Com. 3</th>
                                </tr>
                            </thead>
                            <tbody>
                                {duplicatas.map((d, index) => {
                                    ...
                                    return (
                                        <tr key={`${d.num_docum}-${index}`} className={isAdjusted ? "adjusted" : ""}>
                                            ...
                                            <td>{d.num_docum}</td><td title={d.nome_cliente}>{d.nome_cliente}</td><td title={d.nome_repres_1 || ""}>{d.nome_repres_1?.trim() || "-"}</td><td>{d.pct_comis_1 || 0}%</td><td title={d.nome_repres_2 || ""}>{d.nome_repres_2?.trim() || "-"}</td><td>{d.pct_comis_2 || 0}%</td><td title={d.nome_repres_3 || ""}>{d.nome_repres_3?.trim() || "-"}</td><td>{d.pct_comis_3 || 0}%</td>
                                        </tr>
                                    )
                                })}
                            </tbody>
            ...
            {isConfirmModalOpen && <ConfirmacaoAjusteModal onConfirm={executeAjuste} onCancel={() => setIsConfirmModalOpen(false)} count={duplicatasSelecionadas.size} representantes={representantes} />}
        </div>
    );
};
